...
/**
 * /gkits command - Opens the God Kits category directly.
 */
final class GKitsCommand extends Command implements PluginOwned {
     ...
     * @param Player $player
     */
    private function openGKitsMenu(Player $player): void {
        $kits = $this->kitManager->getKitsByCategory("gkits");

        if (empty($kits)) {
            $player->sendMessage(TF::RED . "No God Kits available!");
            return;
        }
        ...
        $form = new SimpleButtonForm(
            TF::BOLD . TF::GOLD . "God Kits",
            TF::GRAY . "Select a God Kit to claim:",
